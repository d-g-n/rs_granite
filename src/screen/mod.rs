...
impl ScreenContext {
    ...
    pub fn _set_tile(&mut self, x: usize, y: usize, screen_tile: ScreenTile) {
    ...
    pub fn clear(&mut self) {
        for (_y, inner_vec) in self.screen_vec.iter_mut().enumerate() {
            for (_x, mut screen_tile) in inner_vec.iter_mut().enumerate() {
                screen_tile.glyph = 0;
